...
Route::post('user/facebookLogin/', ['uses' =>'EventUserController@FacebookLogin']);
...
Route::get('event/check/{event_id}/{user_id}', function($event_id, $user_id) {
	return EventController::CheckAccessRight($event_id, $user_id);
});
...
Route::get("/event/participant/{event_id}", function($event_id) {
	return js_thai_encode(EventController::EventParticipant($event_id));
});

Route::get("/event/checkedin/{eventId}/{userId}", function($event_id,$user_id) {
	return js_thai_encode(EventController::EventCheckedIn($event_id,$user_id));
});



Route::get("/user/event/approve/{event_id}/{user_id}", function($event_id,$user_id) {
	return js_thai_encode(EventController::EventAddParticipant($event_id,$user_id));
});
...
Route::get("/event/list/{sat}/{sun}/{user_id}", function($sat,$sun,$user_id) {
	return js_thai_encode(EventController::getEventListAllbyUser($sat,$sun,$user_id));
});
...
Route::match(['get', 'post'], 'event/nexthour', ['uses' => 'EventController@Nexthour']);

Route::match(['get', 'post'], 'event/tomorrow', ['uses' => 'EventController@Tomorrow']);
...
Route::get("/event/detail/{event_id}/{user_id}", function($event_id,$user_id) {
	return js_thai_encode(EventController::getEventDetailbyUser($event_id,$user_id));
});
...
//Route::get('/user/info', ['uses' =>'EventUserController@Info']);


Route::get("/user/info/{user_id}", function($user_id) {
	return js_thai_encode(EventUserController::Info($user_id));
});
